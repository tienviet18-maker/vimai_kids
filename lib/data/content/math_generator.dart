...
class MathQuestionGenerator {
  ...
  ContentItem generateBySkill(String skill, {int age = 5}) {
    ...
    switch (skill) {
      ...
      case 'counting':
        return generateCounting(age <= 4 ? 5 : (age == 5 ? 10 : 20));
      case 'missing_number':
      ...
      default:
        return generateNumberRecognition(age <= 3 ? 5 : (age <= 5 ? 10 : 20));
    }
  }
  ...
  ContentItem generateCounting(int maxCount) {
    final n = _random.nextInt(maxCount) + 1;
    return ContentItem(
      ...
      difficulty: 1,
      title: 'Đếm số',
      instruction: 'Có bao nhiêu quả táo?',
      question: List.filled(n, '🍎').join(),
      answer: n.toString(),
      choices: _choicesAround(n, maxCount),
    );
  }
  ...
  ContentItem generateComparison(int maxNum) {
    ...
    return ContentItem(
      ...
      question: '$a  ?  $b',
      answer: answer,
      choices: a == b ? [answer, '$a', '$b'] : ['$a', '$b', 'bằng nhau'],
    );
  }
  ...
  ContentItem generateMissingNumber(int maxNum) {
    ...
    return ContentItem(
      ...
      choices: _choicesAround(missing, maxNum),
    );
  }
  ...
  ContentItem generateWordProblem(int age) {
    ...
    return ContentItem(
      ...
      difficulty: 3,
      title: 'Toán lời văn',
      instruction: 'Bé có $a cái kẹo, thêm $b cái nữa. Bé có bao nhiêu cái?',
      question: '$a + $b = ?',
      answer: (a + b).toString(),
      choices: _choicesAround(a + b, a + b + 5),
    );
  }
  ...
  ContentItem generateBeforeAfter(int maxNum) {
    ...
    return ContentItem(
      ...
      choices: _choicesAround(answer, maxNum),
    );
  }
  ...
  ContentItem generateNumberMatch(int maxNum) {
    ...
    return ContentItem(
      ...
      difficulty: 1,
      title: 'Ghép số',
      instruction: showPicture ? 'Có bao nhiêu chấm?' : 'Chọn nhóm đúng với số $n',
      question: showPicture ? List.filled(n, '•').join() : '$n',
      answer: showPicture ? n.toString() : List.filled(n, '•').join(),
      choices: showPicture
          ? _choicesAround(n, maxNum)
          : _dotChoices(n, maxNum),
    );
  }
  ...
  ContentItem _choiceItem({
    ...
    return ContentItem(
      ...
      question: question,
      answer: answer.toString(),
      choices: _choicesAround(answer, maxValue),
    );
  }

  List<String> _choicesAround(int answer, int maxValue) {
    final choices = <String>{answer.toString()};
    var guard = 0;
    while (choices.length < 4 && guard < 40) {
      guard++;
      final wrong = _random.nextInt(maxValue + 3);
  ...
  List<String> _dotChoices(int answer, int maxNum) {
    final choices = <String>{List.filled(answer, '•').join()};
    var guard = 0;
    while (choices.length < 4 && guard < 40) {
      guard++;
      final n = _random.nextInt(maxNum) + 1;
      choices.add(List.filled(n, '•').join());
    }
    final list = choices.toList()..shuffle(_random);
    return list;
  }
